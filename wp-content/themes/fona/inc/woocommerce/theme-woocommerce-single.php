...
if(!function_exists('zoo_custom_gallery_img_sizes')){
    function zoo_custom_gallery_img_sizes($imgsizes)
    {
        return array_merge($imgsizes, array(
            'zoo_gallery_thumb' => esc_html__('Custom Product Gallery Thumbnail', 'fona'),
        ));
    }
}
...
if ( ! function_exists( 'zoo_product_video' ) ) {
	function zoo_product_video() {
		...
		if ( $zoo_product_video != '' ) {
			...
			$zoo_pv_html = '<div class="wrap-product-video ' . $zoo_embed_class . '"><a href="' . $zoo_product_video . '"  class="button video-lb-control" title="' . esc_attr__( "Watch Video", 'fona' ) . '"><i class="cs-font clever-icon-play-1"></i><span>' . esc_html__( "Watch Video", 'fona' ) . '</span></a>';
...
if(!function_exists('zoo_sale_count_down_variable_html')){
	function zoo_sale_count_down_variable_html($variation){
		...
		if($sale_date > time()) {
			...
			$html .= '<span class="label-product-countdown">'.esc_html__("Hurry Up! Sales end in","fona").'</span>';
			...
			if(get_theme_mod('zoo_product_stock_countdown',1)==1){
				...
				if($zoo_base_stock!=0 && $stock!=NULL) {
	                ...
	                $html .= esc_html__( 'Hurry! Only ', 'fona').'<b class="number-product">'.$stock.'</b>'.esc_html__(' left in Stock.', 'fona' );
	                $html .= '</span>';
...
function zoo_wc_get_stock_html() {
	...
	$label= $availability['class']!='out-of-stock'?esc_html__('In stock','fona'):$availability['availability'];
...
if ( ! function_exists( 'zoo_recently_viewed_product' ) && get_theme_mod( 'zoo_enable_recently_viewed_product', 1 ) == 1 ) {
	function zoo_recently_viewed_product() {
		...
		if ( $the_query->have_posts() ) {
			...
			esc_html_e( 'Recently viewed products', 'fona' );
			echo wp_kses( '</h2>', $allow_html );
...
if ( ! function_exists( 'zoo_sticky_add_to_cart' ) ) {
	function zoo_sticky_add_to_cart() {
		if ( get_theme_mod( 'zoo_enable_sticky_add_to_cart', '1' ) == 1 ) {
			...
			$html .= '</div><a href="#" class="button button-sticky-add-to-cart" title="' . esc_attr__( 'Add to cart', 'fona' ) . '">' . esc_html__( 'Add to cart', 'fona' ) . '</a>';
			$html .= '</div></div>';
			echo ent2ncr( $html );
		}
	}
