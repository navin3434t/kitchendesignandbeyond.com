    ...
    <ul class="zoo-extend-notice">
        ...
        if(get_theme_mod('zoo_product_get_order',1)==1){
            ...
            if ( $availability['class'] != 'out-of-stock' ) {
                ...
                <?php
                echo esc_html__('Order in The Next', 'fona');
                ?>
                        <span class="end-of-day" data-timezone="<?php echo get_option('gmt_offset');  ?>" data-hours="<?php esc_attr_e('hours', 'fona') ?>"
                              data-minutes="<?php esc_attr_e('minutes', 'fona') ?>">
                </span>
                    <?php
                    echo esc_html__(' to get it by', 'fona');
                    ?>
        ...
        if(get_theme_mod('zoo_product_get_visitor',1)==1 && class_exists('CleverOnePixel')){
            ...
            <li class="zoo-extend-notice-item zoo-count-visitor-notice">
                ...
                <span class="extend-notice-label">
                <?php
                esc_html_e('Real Time ','fona');
                echo CleverOnePixel::countVisitors();
                esc_html_e(' Visitor Right Now','fona');
                ?>
            </span>
            </li>
            <?php
            ...
            if ( isset( $min_amounts ) ) {
                if ( is_array( $min_amounts ) && $min_amounts ) {
                    ...
                    if ($current < $min_amount) {
                        echo esc_html__( 'Spend ', 'fona' ) . '<span class="free-shipping-amount">' . wc_price( $min_amount - $current ) . '</span>' . esc_html__( ' to get Free Shipping', 'fona' );
                    }else {
                        esc_html_e('Congratulations! You\'ve got free shipping!', 'fona');
                    }
                    ?>
                    </span>
                    </li>
