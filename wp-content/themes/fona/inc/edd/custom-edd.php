...
function zoo_edd_user_info_fields() {
    ...
    <fieldset id="edd_checkout_user_info">
        <h3><?php echo apply_filters( 'edd_checkout_personal_info_text', esc_html__( 'Personal Info', 'fona' ) ); ?></h3>
		<?php do_action( 'edd_purchase_form_before_email' ); ?>
        <p id="edd-email-wrap">
            <label class="edd-label" for="edd-email">
				<?php esc_html_e( 'Email Address', 'fona' ); ?>
				<?php if ( edd_field_is_required( 'edd_email' ) ) { ?>
                    <span class="edd-required-indicator">*</span>
				<?php } ?>
            </label>
            <span class="edd-description"
                  id="edd-email-description"><?php esc_html_e( 'We will send the purchase receipt to this address.', 'fona' ); ?></span>
        ...
        <p id="edd-first-name-wrap">
            <label class="edd-label" for="edd-first">
				<?php esc_html_e( 'First Name', 'fona' ); ?>
...
function zoo_edd_get_register_fields() {
    ...
    <fieldset id="edd_register_fields">
        ...
        <fieldset id="edd_register_account_fields">
            <h3><?php _e( 'Create an account', 'fona' );
				if ( ! edd_no_guest_checkout() ) {
					echo ' ' . __( '(optional)', 'fona' );
				} ?></h3>
			<?php if ( $show_register_form == 'both' ) { ?>
                <p id="edd-login-account-wrap">
					<?php _e( 'Already have an account?', 'fona' ); ?>
            ...
            <p id="edd-user-login-wrap">
                <label for="edd_user_login">
					<?php _e( 'Username', 'fona' ); ?>
					<?php if ( edd_no_guest_checkout() ) { ?>
                        <span class="edd-required-indicator">*</span>
					<?php } ?>
                </label>
                <span class="edd-description"><?php _e( 'The username you will use to log into your account.', 'fona' ); ?></span>
    ...
    <fieldset id="edd_login_fields">
		...
		<?php if ( $show_register_form == 'both' ) { ?>
            <p id="edd-new-account-wrap">
				<?php _e( 'Need to create an account?', 'fona' ); ?>
                <a href="<?php echo esc_url( remove_query_arg( 'login' ) ); ?>" class="edd_checkout_register_login"
                   ...
                   data-nonce="<?php echo wp_create_nonce( 'edd_checkout_register' ); ?>">
					<?php _e( 'Register', 'fona' );
					if ( ! edd_no_guest_checkout() ) {
						echo ' ' . __( 'or checkout as a guest.', 'fona' );
					} ?>
                </a>
            </p>
		<?php } ?>
...
function zoo_edd_checkout_submit() {
    ...
    <fieldset id="edd_purchase_submit">
		...
		<?php if ( edd_is_ajax_disabled() ) { ?>
            <p class="edd-cancel"><a href="<?php echo edd_get_checkout_uri(); ?>"><?php _e( 'Go back', 'fona' ); ?></a></p>
		<?php } ?>
    </fieldset>
	<?php
}
