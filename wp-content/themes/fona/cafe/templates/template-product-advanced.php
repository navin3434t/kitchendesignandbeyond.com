        ...
        <?php if($settings['tabs_filter'] == 'asset') { ?>
            <ul class="cafe-ajax-load filter-asset">
              ...
              switch ( $settings['default_asset'] ) {
                 case 'featured':
                 $asset_title = esc_html__( 'Featured', 'fona' );
                 break;
                 case 'onsale':
                 $asset_title = esc_html__( 'On Sale', 'fona' );
                 break;
                 case 'deal':
                 $asset_title = esc_html__( 'Deal', 'fona' );
                 break;
                 case 'latest':
                 $asset_title = esc_html__( 'New Arrivals', 'fona' );
                 break;
                 case 'best-selling':
                 $asset_title = esc_html__( 'Best Seller', 'fona' );
                 break;
                 case 'toprate':
                 $asset_title = esc_html__( 'Top Rate', 'fona' );
                 break;
                 default:
                 break;
             }
            ...
            if ( $settings['filter_assets'] ) {
             ...
             foreach ( $settings['filter_assets'] as $val ) {
                switch ( $val ) {
                   case 'featured':
                   $html .= $settings['default_asset'] != 'featured' ? '<li><a href="#" data-type="asset_type" data-value="featured" title="' . esc_html__( 'Featured', 'fona' ) . '">' . esc_html__( 'Featured', 'fona' ) . '</a></li>' : '';
                   break;
                   case 'onsale':
                   $html .= $settings['default_asset'] != 'onsale' ? '<li><a href="#" data-type="asset_type" data-value="onsale" title="' . esc_html__( 'On Sale', 'fona' ) . '">' . esc_html__( 'On Sale', 'fona' ) . '</a></li>' : '';
                   break;
                   case 'deal':
                   $html .= $settings['default_asset'] != 'deal' ? '<li><a href="#" data-type="asset_type" data-value="deal" title="' . esc_html__( 'Deal', 'fona' ) . '">' . esc_html__( 'Deal', 'fona' ) . '</a></li>' : '';
                   break;
                   case 'latest':
                   $html .= $settings['default_asset'] != 'latest' ? '<li><a href="#" data-type="asset_type" data-value="latest" title="' . esc_html__( 'New Arrivals', 'fona' ) . '">' . esc_html__( 'New Arrivals', 'fona' ) . '</a></li>' : '';
                   break;
                   case 'best-selling':
                   $html .= $settings['default_asset'] != 'best-selling' ? '<li><a href="#" data-type="asset_type" data-value="best-selling" title="' . esc_html__( 'Best Seller', 'fona' ) . '">' . esc_html__( 'Best Seller', 'fona' ) . '</a></li>' : '';
                   break;
                   case 'toprate':
                   $html .= $settings['default_asset'] != 'toprate' ? '<li><a href="#" data-type="asset_type" data-value="toprate" title="' . esc_html__( 'Top Rate', 'fona' ) . '">' . esc_html__( 'Top Rate', 'fona' ) . '</a></li>' : '';
                   break;
                   default:
                   break;
               }
...
<?php if($settings['pagination'] != 'none' && $settings['tabs_filter'] == 'none' && $settings['layout'] == 'grid') { ?>
  ...
  <?php if($zoo_pag_type == 'ajaxload' || $zoo_pag_type == 'infinity') { wp_enqueue_script('infinite-scroll'); ?>
    ...
    <?php } ?>
    <?php
    cafe_pagination(3, $product_query, '', '<i class="zoo-icon-long-arrow-left"></i> ' . esc_html__('', 'fona'), esc_html__('', 'fona') . ' <i class="zoo-icon-long-arrow-right"></i>');
    ?>
        <?php if($zoo_pag_type == 'ajaxload' || $zoo_pag_type == 'infinity') { ?>

            <div class="scroller-status">
                <div class="infinite-scroll-request">
                    <div class="pagination-loading"><span class="loading"><?php echo esc_html__('LOADING...','fona') ?></span></div>
                </div>
                <p class="infinite-scroll-last"><span><?php echo esc_html__('All items loaded','fona'); ?></span></p>
                <p class="infinite-scroll-error"><span><?php echo esc_html__('No more page','fona'); ?></span></p>
            </div>
            <?php if ($zoo_pag_type == 'ajaxload') { ?>
                <p class="view-more-button"><span><?php echo esc_html__('Load More products','fona') ?></span></p>
            <?php } ?>
        </div>
    <?php } ?>
<?php } ?>
