...
?>
<div class="measurement_filter">
<h3>HOW MUCH DO YOU NEED?</h3>
<div class="prc_calc">
<div id="price_calculator" class="wc-measurement-price-calculator-price-table <?php echo sanitize_html_class( $product->get_type() . '_price_calculator' ) . ' ' . sanitize_html_class( $calculator_mode ); ?>">
<h3><i class="fa fa-calculator"></i> SQFT TO BOXES CALCULATOR</h3>
<div class="row">
...
<input
				type="text"
				id="_measure_needed"
				name="_measure_needed"
				value=""
			/>
			<label for="contingency" class="sqrLabel">
							SqFt
	...
	<?php /* foreach ( $measurements as $measurement ) : ?>
		...
		?>
		<span class="cross_measure"> x </span>
		<div class="price-table-row <?php echo sanitize_html_class( $measurement->get_name() ); ?>-input">
...
<label for="<?php echo esc_attr( $measurement_name ); ?>">
	...
	<?php if ( $settings->is_calculator_type_derived() ) : ?>
		...
		<div class="price-table-row total-amount">
				...
				<span
					...
					<!--input type="text" id="customft" class="wc-measurement-price-calculator-total-amount" data-unit="<?php echo esc_attr( $product_measurement->get_unit() ); ?>"-->
					<label><?php echo $total_amount_text; ?></label>
			</div>
	...
	<?php if ( $has_pricing_overage ) : ?>
		...
		<div class="price-table-row calculated-price-overage">
			<!--div><?php echo esc_html( sprintf( __( 'Overage estimate (%s%%)', 'woocommerce-measurement-price-calculator' ), $pricing_overage * 100 ) ); ?></div-->
				<span class="product_price_overage"></span>
				<label>
				Add <?php echo $pricing_overage * 100;?>% for contingency (recommended)
				<input type="checkbox" class="perct">
	...
	<div class="price-table-row calculated-price">
		
		<?php echo esc_html( $has_pricing_overage ? __( 'Total Price', 'woocommerce-measurement-price-calculator' ) : __( 'Product Price', 'woocommerce-measurement-price-calculator' ) ); ?>
...
<div class="col-lg-5 col-md-5 col-sm-5"><div class="icon"><i class="fa fa-calculator"></i></div></div></div>
</div>
</div>
</div>


<div id="feetperbox">1 piece =<span id="productarea"> <?php echo $product->get_meta( '_area' );?></span> <span class="itemCalcPrice"> sqft |  <?php $price = $product->get_price(); echo woocommerce_price($price);?></span></div>
<div class="box_pieces" data-piece="<?php echo $product->get_meta( 'box_pieces' );?>">
<strong>Pieces in a Box: </strong><span><?php echo $product->get_meta( 'box_pieces' );?></span>
</div>
<div class="quantity_heading">Quantity of Boxes</div>
<?php
