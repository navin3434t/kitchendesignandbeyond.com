    ...
    <div class="cvca-wrapper-testimonial-block <?php if ($atts['style'] == 'carousel') {
        ...
        while ($the_query->have_posts()):$the_query->the_post();
            ...
            <article id="testimonial-<?php the_ID(); ?>" <?php echo post_class('cvca-testimonial-item ' . $class) ?>>
                <?php
                if (get_post_meta(get_the_ID(), 'cvca_author_img', true) != '' && !$atts['hide_avatar']) {
                    ?>
                    <div class="cvca-wrap-avatar">
                        <img
                                src="<?php echo wp_get_attachment_image_url(get_post_meta(get_the_ID(), 'cvca_author_img', true), 'full') ?>"
                ...
                <div class="cvca-wrap-author-info">
                    <?php
                    if (get_post_meta(get_the_ID(), 'cvca_author', true) != '') { ?>
                        <h4 class="cvca-testimonial-author"><?php
                            echo esc_html(get_post_meta(get_the_ID(), 'cvca_author', true)); ?>
                        </h4>
                    <?php } ?>
                    <?php
                    if (get_post_meta(get_the_ID(), 'cvca_author_des', true) != '') { ?>
                        <div class="cvca-testimonial-des"><?php
                            echo esc_html(get_post_meta(get_the_ID(), 'cvca_author_des', true)); ?>
                        </div>
                        <?php
                    } ?>
                </div>
