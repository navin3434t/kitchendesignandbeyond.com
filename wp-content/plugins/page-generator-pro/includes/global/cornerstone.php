<?php
/**
 * Cornerstone class. Registers shortcodes as Elements for Pro and X Themes.
 * 
...
class Page_Generator_Pro_Cornerstone {
    ...
    public function __construct( $base ) {
        ...
        add_action( 'page_generator_pro_generate_content_settings', array( $this, 'convert_elements_to_class_raw_content_elements' ), 10, 4 );
        add_action( 'page_generator_pro_generate_content_finished', array( $this, 'rebuild_post_content' ), 10, 5 );
    ...
    public function add_elements() {
        ...
        foreach ( $shortcodes as $shortcode => $shortcode_properties ) {
            ...
            // Register Element
            cs_register_element( $this->base->plugin->name . '-' . $shortcode, array(
    ...
    private function get_element_controls( $shortcode, $tabs, $fields, $default_values ) {
        ...
        foreach ( $tabs as $tab_name => $tab_properties ) {
            ...
            foreach ( $tab_properties['fields'] as $field_name ) {
                ...
                $group_controls[] = $this->get_element_control( $field_name, $fields[ $field_name ], $default_value );
            }
     ...
     */
    private function get_element_control( $field_name, $field, $default_value ) {
        ...
        if ( isset( $field['condition'] ) ) {
            $element_control = array_merge( $element_control, array(
                'condition' => array(
    ...
    public function render_element( $atts ) {
        ...
        // Output shortcode
        ob_start();
        echo '<div  class="x-raw-content" >' . $html . '</div>';
        return ob_get_clean();
     ...
     */
    public function convert_elements_to_class_raw_content_elements( $settings, $group_id, $index, $test_mode ) {
        ...
        $settings['post_meta']['_cornerstone_data'] = json_encode( $data, JSON_UNESCAPED_SLASHES );
     ...
     * @param   bool    $test_mode      Test Mode
     */
    public function rebuild_post_content( $post_id, $group_id, $settings, $index, $test_mode ) {
