<?php
/**
 * Media Library Shortcode/Block class
 * 
 * @package Page_Generator_Pro
 * @author  Tim Carr
 * @version 2.5.1
 */
class Page_Generator_Pro_Shortcode_Media_Library {
    ...
    public function get_overview() {
        ...
        return array(
            ...
            'description'   => __( 'Displays an image from the Media Library', $this->base->plugin->name ),
            'icon'      => file_get_contents( $this->base->plugin->folder . '/assets/images/icons/admin-media.svg' ),
            'category'  => $this->base->plugin->name,
            'keywords'  => array(
                __( 'Media Library', $this->base->plugin->name ),
                __( 'Media', $this->base->plugin->name ),
                __( 'Library', $this->base->plugin->name ),
    ...
    public function render( $atts ) {
        ...
        if ( ! $image_id ) {
            if ( defined( 'PAGE_GENERATOR_PRO_DEBUG' ) && PAGE_GENERATOR_PRO_DEBUG === true ) {
                return __( 'Media Library Shortcode Error: No image could be found.', 'page-generator-pro' );
            }
        ...
        if ( $atts['copy'] ) {
            ...
            if ( ! $image ) {
                if ( defined( 'PAGE_GENERATOR_PRO_DEBUG' ) && PAGE_GENERATOR_PRO_DEBUG === true ) {
                    return __( 'Media Library Shortcode Error: Could not copy image.', 'page-generator-pro' );
                }
            ...
            if ( is_wp_error( $image_id ) ) {
                if ( defined( 'PAGE_GENERATOR_PRO_DEBUG' ) && PAGE_GENERATOR_PRO_DEBUG === true ) {
                    return sprintf( __( 'Media Library Shortcode Error: %s', 'page-generator-pro' ), $image_id->get_error_message() );
                }
        ...
        if ( is_wp_error( $exif ) ) {
            if ( defined( 'PAGE_GENERATOR_PRO_DEBUG' ) && PAGE_GENERATOR_PRO_DEBUG === true ) {
                return sprintf( __( 'Media Library Shortcode Error: %s', 'page-generator-pro' ), $exif->get_error_message() );
            }
        ...
        // Get image HTML
        $html = $this->base->get_class( 'media_library' )->get_image_html_tag_by_id( $image_id, array(
            'size'      => $atts['size'],
