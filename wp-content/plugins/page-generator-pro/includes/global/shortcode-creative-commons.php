<?php
/**
 * Creative Commons Shortcode/Block class
 * 
 * @package Page_Generator_Pro
 * @author  Tim Carr
 * @version 2.6.9
 */
class Page_Generator_Pro_Shortcode_Creative_Commons {
    ...
    public function get_overview() {
        ...
        return array(
            'title'     => __( 'Creative Commons', $this->base->plugin->name ),
            'description'   => __( 'Displays an image from Creative Commons, based on the given search parameters.', $this->base->plugin->name ),
            'icon'      => file_get_contents( $this->base->plugin->folder . '/assets/images/icons/creative-commons.svg' ),
            'category'  => $this->base->plugin->name,
            'keywords'  => array(
                __( 'Creative Commons', $this->base->plugin->name ),
    ...
    public function render( $atts ) {
        ...
        // Run images query
        $images = $this->base->get_class( 'creative_commons' )->photos_search( $atts['term'], $atts['size'], $atts['orientation'] );
        if ( is_wp_error( $images ) ) {
            ...
            if ( defined( 'PAGE_GENERATOR_PRO_DEBUG' ) && PAGE_GENERATOR_PRO_DEBUG === true ) {
                return sprintf( __( 'creative_commons Shortcode Error: %s', 'page-generator-pro' ), $images->get_error_message() );
            }
        ...
        if ( is_wp_error( $image_id ) ) {
            if ( defined( 'PAGE_GENERATOR_PRO_DEBUG' ) && PAGE_GENERATOR_PRO_DEBUG === true ) {
                return sprintf( __( 'Creative Commons Shortcode Error: %s', 'page-generator-pro' ), $image_id->get_error_message() );
            }
        ...
        if ( is_wp_error( $exif ) ) {
            if ( defined( 'PAGE_GENERATOR_PRO_DEBUG' ) && PAGE_GENERATOR_PRO_DEBUG === true ) {
                return sprintf( __( 'Creative Commons Shortcode Error: %s', 'page-generator-pro' ), $exif->get_error_message() );
            }
         ...
         */
        $html = apply_filters( 'page_generator_pro_shortcode_creative_commons', $html, $atts, $image_id, $images );
