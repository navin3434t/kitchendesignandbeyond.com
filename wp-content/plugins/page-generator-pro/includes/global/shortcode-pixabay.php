<?php
/**
 * Pixabay Shortcode/Block class
 * 
 * @package Page_Generator_Pro
 * @author  Tim Carr
 * @version 2.5.1
 */
class Page_Generator_Pro_Shortcode_Pixabay {
    ...
    public function get_overview() {
        ...
        return array(
            ...
            'description'   => __( 'Displays an image from Pixabay, based on the given search parameters.', $this->base->plugin->name ),
            'icon'      => file_get_contents( $this->base->plugin->folder . '/assets/images/icons/pixabay.svg' ),
            'category'  => $this->base->plugin->name,
            'keywords'  => array(
                __( 'Pixabay', $this->base->plugin->name ),
    ...
    public function render( $atts ) {
        ...
        if ( is_wp_error( $exif ) ) {
            if ( defined( 'PAGE_GENERATOR_PRO_DEBUG' ) && PAGE_GENERATOR_PRO_DEBUG === true ) {
                return sprintf( __( 'Pixabay Shortcode Error: %s', 'page-generator-pro' ), $exif->get_error_message() );
            }
