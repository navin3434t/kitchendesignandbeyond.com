...
class Page_Generator_Pro_Groups_Terms {
    ...
    public function get_all() {
        ...
        // Groups
        $groups = new WP_Term_Query( array(
    ...
    public function run_action( $action, $id, $redirect = false ) {
        ...
        switch ( $action ) {
            ...
            case 'generate':
                ...
                if ( $result ) { 
                    wp_redirect( 'admin.php?page=' . $this->base->plugin->name . '-generate&id=' . $id . '&type=term' );
                    die();
                }
                break;
        ...
        if ( is_wp_error( $result ) ) {
            ...
            if ( $redirect ) {
                ...
                wp_redirect( 'edit-tags.php?taxonomy=' . $this->base->get_class( 'taxonomy' )->taxonomy_name );
                die();
            }
        ...
        switch ( $action ) {
             ...
             */
            case 'test':
                $message = __( 'Test Term Generated in ' . $result['duration'] . ' seconds at ', 'page-generator-pro' ) . ': ' . '<a href="' . $result['url'] . '" rel="noopener" target="_blank">' . $result['url'] . '</a>';
    ...
    public function validate( $id ) {
        ...
        if ( ! $settings ) {
            return new WP_Error( 'page_generator_pro_groups_terms_validate', sprintf( __( 'Group ID %s does not exist!', 'page-generator-pro' ), $id ) );
        }
    ...
    public function schedule_generation( $id ) {
        ...
        if ( defined( 'DISABLE_WP_CRON' ) && DISABLE_WP_CRON ) {
            return new WP_Error( 
                ...
                sprintf(
                    __( 'Generate via Server failed, because WordPress\' Cron is disabled due to DISABLE_WP_CRON enabled in your wp-config.php 
                        file. <a href="%s" rel="noopener" target="_blank">Remove this option in your wp-config.php file</a>, or use 
                        <a href="%s" rel="noopener" target="_blank">WP-CLI</a>.', 'page-generator-pro' ),
                    $this->base->plugin->documentation_url . '/generate-server/',
                    $this->base->plugin->documentation_url . '/generate-wp-cli/#generate-content'
                )
            );
        }
        ...
        if ( $this->is_scheduled( $id ) ) {
            return new WP_Error( 'page_generator_pro_groups_terms_schedule_generation', sprintf( __( 'Group #%s: is already scheduled to generate content!', 'page-generator-pro' ), $id ) );
        }

        // Bail if the group is already generating content
        if ( $this->is_generating( $id ) ) {
            return new WP_Error( 'page_generator_pro_groups_terms_schedule_generation', sprintf( __( 'Group #%s: is already generating content!', 'page-generator-pro' ), $id ) );
        }
    ...
    public function duplicate( $id ) {
        ...
        // Create new Term
        $duplicate_term_id = wp_insert_term( $settings['title'] . __( ' - Copy', 'page-generator-pro' ), $term->taxonomy, array(
    ...
    public function has_generated_content( $id ) {
        ...
        $generated_pages_count = $this->get_generated_count_by_id( $id, $settings['taxonomy'] );
    ...
    public function cancel_generation_requested( $id ) {
        ...
        $result = $wpdb->get_var( " SELECT meta_value FROM " . $wpdb->termmeta . "
