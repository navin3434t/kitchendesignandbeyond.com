...
class Page_Generator_Pro_Spintax {
     ...
     * @param   bool    $test_mode  Test Mode (true = return error)
     * @return  mixed               WP_Error | string
     */
    public function add_spintax( $content, $test_mode = false ) {
    ...
    private function add_spintax_wordai( $content, $protected_words ) {
        ...
        // Build API compatible parameters
        $params = array(
            'quality'    => $this->base->get_class( 'settings' )->get_setting( $this->base->plugin->name . '-spintax', 'wordai_confidence_level', 'low' ),
            'nonested'   => $this->base->get_class( 'settings' )->get_setting( $this->base->plugin->name . '-spintax', 'wordai_nested_spintax', 1 ),
            'sentence'   => $this->base->get_class( 'settings' )->get_setting( $this->base->plugin->name . '-spintax', 'wordai_spin_sentences', 1 ),
            'paragraph'  => $this->base->get_class( 'settings' )->get_setting( $this->base->plugin->name . '-spintax', 'wordai_spin_paragraphs', 1 ),
        );
    ...
    private function add_spintax_local( $content, $protected_words ) {
        ...
        foreach ( $this->get_synonyms() as $synonyms ) {
            foreach ( $synonyms as $word ) {
                ...
                if ( in_array( $word, $protected_words ) ) {
                    continue;
                }

                // Skip if the entire word doesn't exist in the content
                if ( ! preg_match( '/\b\s' . $word . '\s\b/' . $regex_modifier, $content ) ) {
                    continue;
                }

                // Replace the word with the synonyms
