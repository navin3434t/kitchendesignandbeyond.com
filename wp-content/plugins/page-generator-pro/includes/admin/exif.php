...
class Page_Generator_Pro_Exif {
    ...
    */
    public function write( $image_id, $description = false, $comment = false, $latitude = false, $longitude = false ) {
        ...
        if ( ! in_array( $file_type['type'], $this->get_supported_file_types() ) ) {
            return true;
        }

        try {
            // Read Exif data
            $data = new lsolesen\pel\PelDataWindow( file_get_contents( $image ) );

            // Determine if it's JPEG or TIFF data
            if ( lsolesen\pel\PelJpeg::isValid( $data ) ) {
                $jpeg = $file = new lsolesen\pel\PelJpeg();
                $jpeg->load( $data );
                ...
                $tiff = $file = new lsolesen\pel\PelTiff();
                $tiff->load( $data );
            } else {
            ...
            if ( isset( $exif['camera_make'] ) ) {
                $this->set_camera_make( $exif['camera_make'], $ifd0 );
            }

            // Camera Model
            if ( isset( $exif['camera_model'] ) ) {
                $this->set_camera_model( $exif['camera_model'], $ifd0 );
            }

            // Created Timestamp
            if ( isset( $exif['created_date'] ) && isset( $exif['created_time'] ) ) {
                $this->set_created_timestamp( strtotime( $exif['created_date'] . ' ' . $exif['created_time'] ), $subifd );
            }

            // Author
            if ( isset( $exif['author'] ) ) {
                $this->set_author( $exif['author'], $ifd0 );
            }
            
            // Copyright
            if ( isset( $exif['copyright'] ) ) {
                $this->set_copyright( $exif['copyright'], $ifd0 );
            }

            // Title
            /*
            if ( isset( $exif['title'] ) ) {
                $this->set_title( $exif['title'], $ifd0 );
            }
            */
    ...
    private function convert_latitude_degrees_minutes_seconds_to_decimal( $degrees, $minutes, $seconds, $ref ) {
        ...
        // Get latitude
        $latitude = $this->convert_degrees_minutes_seconds_to_decimal( $degrees, $minutes, $seconds, $ref );
    ...
    private function convert_longitude_degrees_minutes_seconds_to_decimal( $degrees, $minutes, $seconds, $ref ) {
        ...
        // Get longitude
        $longitude = $this->convert_degrees_minutes_seconds_to_decimal( $degrees, $minutes, $seconds, $ref );
     ...
     */
    private function convert_degrees_minutes_seconds_to_decimal( $degrees, $minutes, $seconds, $ref ) {
