...
class Page_Generator_Pro_Groups_Table {
    ...
    public function run_bulk_actions( $redirect_to, $action, $post_ids ) {
        ...
        switch ( $action ) {
            ...
            case 'duplicate':
                foreach ( $post_ids as $post_id ) {
                    ...
                    if ( is_wp_error( $result ) ) {
                        ...
                        $this->base->get_class( 'notices' )->add_success_notice( sprintf( __( 'Group #%s: %s', 'page-generator-pro' ), $post_id, $this->base->get_class( 'groups_ui' )->get_title( 'duplicate_success' ) ) );
                    }
                }
                break;
            ...
            case 'generate_server':
                foreach ( $post_ids as $post_id ) {
                    ...
                    if ( is_wp_error( $result ) ) {
                        ...
                        $this->base->get_class( 'notices' )->add_success_notice( sprintf( __( 'Group #%s: %s', 'page-generator-pro' ), $post_id, $this->base->get_class( 'groups_ui' )->get_title( 'generate_server_success' ) ) );
                    }
                }
                break;
            ...
            case 'trash_generated_content':
                foreach ( $post_ids as $post_id ) {
                    ...
                    if ( is_wp_error( $result ) ) {
                        ...
                        $this->base->get_class( 'notices' )->add_success_notice( sprintf( __( 'Group #%s: %s', 'page-generator-pro' ), $post_id, $this->base->get_class( 'groups_ui' )->get_title( 'generate_trash_generated_content_success' ) ) );
                    }
                }
                break;
            ...
            case 'delete_generated_content':
                foreach ( $post_ids as $post_id ) {
                    ...
                    if ( is_wp_error( $result ) ) {
                        ...
                        $this->base->get_class( 'notices' )->add_success_notice( sprintf( __( 'Group #%s: %s', 'page-generator-pro' ), $post_id, $this->base->get_class( 'groups_ui' )->get_title( 'generate_delete_generated_content_success' ) ) );
                    }
                }
                break;
            ...
            default:
                 ...
                 */
                do_action( 'page_generator-pro_groups_table_run_bulk_actions', $action, $post_ids );
                break;
    ...
    public function admin_columns_output( $column_name, $post_id ) {
        ...
        switch ( $column_name ) {
            ...
            case 'generated_count':
                if ( $this->base->get_class( 'groups' )->has_generated_content( $post_id ) ) {
                    $items = array(
                        ...
                        'spacer'                    => '&nbsp;',
                        'view'                      => '<a href="' . admin_url( 'edit.php?post_type=' . $this->settings['type'] . '&page_generator_pro_group_id=' . $this->group_id ) .'" title="' . __( 'Click to view Generated Content', 'page-generator-pro' ) . '">' . 
                            __( 'View Generated Content', 'page-generator-pro' ) . 
                        '</a>',
                        'trash_generated_content'   => '<a href="' . admin_url( 'edit.php?post_type=' . $this->base->get_class( 'post_type' )->post_type_name . '&' . $this->base->plugin->name . '-action=trash_generated_content&id=' . $post_id . '&type=content' ) . '">' . 
                            __( 'Trash Generated Content', 'page-generator-pro' ) . 
                        '</a>',
                        'delete_generated_content'  => '<a href="' . admin_url( 'edit.php?post_type=' . $this->base->get_class( 'post_type' )->post_type_name . '&' . $this->base->plugin->name . '-action=delete_generated_content&id=' . $post_id . '&type=content' ) . '">' . 
                            __( 'Delete Generated Content', 'page-generator-pro' ) . 
                        '</a>',
                    );
                } else {
                    $items = array(
                        'generated_count'           => sprintf( __( 'Generated: %s', 'page-generator-pro' ), 0 ),
                        'last_index_generated'      => sprintf( __( 'Last Index Generated: %s', 'page-generator-pro' ), $this->settings['last_index_generated'] ),
                    );
                }
                break;
            ...
            case 'status':
                if ( $this->base->get_class( 'groups' )->is_idle( $post_id ) ) {
                    $items = array(
                        'test'              => '<a href="edit.php?post_type=' . $this->base->get_class( 'post_type' )->post_type_name . '&' . $this->base->plugin->name . '-action=test&id=' . $post_id . '&type=content">' . $this->base->get_class( 'groups_ui' )->get_title( 'test' ) . '</a>',
                        'generate'          => '<a href="admin.php?page=' . $this->base->plugin->name . '-generate&id=' . $post_id . '&type=content">' . $this->base->get_class( 'groups_ui' )->get_title( 'generate' ) . '</a>',
                        'generate_server'   => '<a href="edit.php?post_type=' . $this->base->get_class( 'post_type' )->post_type_name . '&' . $this->base->plugin->name . '-action=generate_server&id=' . $post_id . '&type=content">' . $this->base->get_class( 'groups_ui' )->get_title( 'generate_server' ) . '</a>',
                    );
                } else {
                    $items = array(
                        ...
                        'cancel_generation' => '<a href="edit.php?post_type=' . $this->base->get_class( 'post_type' )->post_type_name . '&' . $this->base->plugin->name . '-action=cancel_generation&id=' . $post_id . '&type=content">' . $this->base->get_class( 'groups_ui' )->get_title( 'cancel_generation' ) . '</a>',
                    );
                }
                break;
            ...
            default:
                 ...
                 */
                $items = apply_filters( 'page_generator_pro_groups_table_admin_columns_output', $items, $column_name, $post_id );
                break;
        }
        ...
        foreach ( $items as $class => $item ) {
            echo '<span class="' . $class . '">' . $item . '</span><br />';
        }
    ...
    public function admin_row_actions( $actions ) {
        ...
        $actions['duplicate'] = '<a href="edit.php?post_type=' . $this->base->get_class( 'post_type' )->post_type_name . '&' . $this->base->plugin->name . '-action=duplicate&id=' . $post->ID . '&type=content">' . $this->base->get_class( 'groups_ui' )->get_title( 'duplicate' ) . '</a>';
