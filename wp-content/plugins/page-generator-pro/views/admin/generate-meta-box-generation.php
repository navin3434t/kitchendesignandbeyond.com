...
<div class="wpzinc-option sidebar">
	...
	<p class="description">
		...
		echo sprintf( 
			...
			'<a href="' . $this->base->plugin->documentation_url . '/generate-content/#fields--generation--overwrite" rel="noopener" target="_blank">' . __( 'Documentation', 'page-generator-pro' ) . '</a>'
		);
		?>
	</p>
</div>
...
if ( isset( $overwrite_sections ) ) {
	...
	<div class="wpzinc-option sidebar overwrite-sections overwrite overwrite_any overwrite_preserve_date overwrite_any_preserve_date">
		...
		<p class="description">
			<?php
			echo sprintf( 
				__( 'If generation would overwrite an existing %s, choose which items of the existing %s to overwrite.', 'page-generator-pro' ),
				$labels['singular'],
				$labels['singular']
			);
			?>
...
<div class="wpzinc-option sidebar">
	<div class="left">
		<label for="number_of_posts"><?php echo sprintf( __( 'No. %s', 'page-generator-pro' ), $labels['plural'] ); ?></label>
	</div>
	<div class="right">
