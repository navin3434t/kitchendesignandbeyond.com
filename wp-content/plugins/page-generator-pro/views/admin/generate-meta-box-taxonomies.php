...
switch ( $taxonomy->hierarchical ) {
	...
	case true:
		...
		?>
		<div class="wpzinc-option taxonomy <?php echo trim( $post_types_string ); ?>">
			<div class="left">
			...
			<div class="full">
				<input type="text" name="<?php echo $this->base->plugin->name; ?>[tax][<?php echo $taxonomy->name; ?>][0]" value="<?php echo ( isset( $this->settings['tax'][ $taxonomy->name ][0] ) ? $this->settings['tax'][ $taxonomy->name ][0] : '' ); ?>" class="widefat wpzinc-autocomplete" placeholder="<?php _e( 'Enter new taxonomy terms to create here.', 'page-generator-pro' ); ?>" />
			</div>
		</div>
		<?php
		break;
	...
	case false:
		...
		<div class="wpzinc-option taxonomy <?php echo trim( $post_types_string ); ?>">
			...
			<div class="full">
				<input type="text" name="<?php echo $this->base->plugin->name; ?>[tax][<?php echo $taxonomy->name; ?>]" value="<?php echo ( isset( $this->settings['tax'][ $taxonomy->name ] ) ? $this->settings['tax'][ $taxonomy->name ] : '' ); ?>" class="widefat wpzinc-autocomplete" />
			</div>
		</div>
		<?php
		break;
