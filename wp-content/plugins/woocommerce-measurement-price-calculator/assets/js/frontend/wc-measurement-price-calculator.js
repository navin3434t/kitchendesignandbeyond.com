...
*/
totalMeasurement = $( '#_measure_needed:input' ).val();
//console.log(totalMeasurement);
			...
			var price              = 0.0,
				...
				if($( '.perct' ).prop("checked") == true){
					var per = overage_percentage * 100;
					var t = (totalMeasurement * per)/100;
					totalMeasurement +=t;
					//console.log(totalMeasurement);
				}
			if ( totalMeasurement ) {
...
if(totalMeasurement <= 9){
	quantity = 1;
}else{
				box_piece = $('.box_pieces').attr('data-piece');
				quantity = totalMeasurement / box_piece;
				quantity = Math.round(quantity);
				console.log(box_piece);
				// update the quantity
				}
				$( 'input[name=quantity]' ).val( quantity );

				//console.log(wc_price_calculator_params);
				//console.log(wc_price_calculator_params.product_measurement_unit);
				//console.log(wc_price_calculator_params.product_total_measurement_common_unit );
		...
		// pricing calculator measurement changed: update product pricing
		$( '#_measure_needed:input' ).on( 'keyup change mpc-change', function() {
			var $cart = $( this ).closest( 'form.cart' );
			$cart.trigger( 'wc-measurement-price-calculator-update' );
		...
		// pricing calculator measurement changed: update product pricing
		$( '.perct' ).on( 'change mpc-change', function() {
			var $cart = $( this ).closest( 'form.cart' );
			$cart.trigger( 'wc-measurement-price-calculator-update' );
